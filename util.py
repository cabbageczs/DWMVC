...
def set_seed(seed_num):
    ...
    # torch.random.manual_seed(seed_num + 3)
...
def getMvKNNGraph(X, k=5, mode='connectivity'):
    MvG = []
    for x in X:
        subG = kneighbors_graph(x.cpu(), n_neighbors=k, n_jobs=-1, include_self=False, mode=mode)
        subG = subG.toarray()
...
def plot_embedding_2d(data, labels):
    ...
    # plt.legend()
    plt.show()
    plt.savefig('/home/weidu/temp/IMVC/TSNE/CIFAR10.png', dpi=300)
...
def euclidean_dist(x, y, root=True):
    """
    Args:
